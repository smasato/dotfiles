...
キーバインドではないが、`wt` で worktree を作成すると post-start hook（`scripts/worktree-open.sh`）が
その worktree を Herdr で開き、ワークスペースを自動レイアウトする:
タブ 1 = 「左: シェル / 右: ファイルビューア」の split、タブ 2 = lazygit、
タブ 3 = 「左: hunk diff（worktree、未ステージ含む作業ツリー）/ 右: hunk diff（branch、upstream からのブランチ全体の差分）」の split。
フォーカスはタブ 1 のシェルに残る。
