...
- `chezmoi diff` hides always-run scripts; `chezmoi apply` still executes them. When changing these scripts or `.chezmoi.toml.tmpl`, see [diff configuration and inspection](README.md#chezmoi-diff).
...
- `/dot_claude_work/`: Work-only (`.work`) Claude home. Symlinks into `~/.claude` (settings, CLAUDE.md, agents, hooks, rules) and `~/.agents/skills` so the `wclaude` alias (`CLAUDE_CONFIG_DIR=~/.claude_work`) shares config while keeping its own authentication and sessions. Plain `claude` uses the personal account in the standard home. See `docs/claude-accounts.md` when switching from the old `pclaude` layout
- `/dot_codex/`: Codex CLI configuration for the personal account (`~/.codex`) - global AGENTS.md overlay and `config.toml` (modify-template; existing settings take precedence over defaults). The modify-template is copied byte-identical into `dot_codex_work1` / `dot_codex_work2` / `dot_codex_work3` because chezmoi modify-templates cannot include `.chezmoitemplates`
- `/dot_codex_work1/`, `/dot_codex_work2/`, `/dot_codex_work3/`: Work-only (`.work`) Codex homes for the three Business seats. Symlinks into `~/.codex` (AGENTS.md, rules, skills, plugins, hooks.json) plus their own `config.toml` so the `wcodex1` / `wcodex2` / `wcodex3` aliases (`CODEX_HOME`) share config while keeping separate `auth.json` / sessions
