...
if (process.cwd() !== ${JSON.stringify(realpathSync(join(directory, 'repo')))}) process.exit(2);
const args = process.argv.slice(2);
if (args.join(" ") === "stack view --json") {
  process.stdout.write(readFileSync(${JSON.stringify(outputPath)}, "utf8"));
} else if (args[0] === "pr" && args[1] === "view") {
  const branches = {"10":"stack/merged", "13":"stack/closed", "11":"stack/open"};
  const branch = branches[args[2]];
  if (!branch) process.exit(3);
  console.log(JSON.stringify({number:Number(args[2]), headRefName:branch,
    headRefOid:execFileSync("git",["rev-parse",branch],{encoding:"utf8"}).trim(),
    state:args[2] === "10" ? "MERGED" : args[2] === "13" ? "CLOSED" : "OPEN"}));
} else process.exit(2);
`,
  );
...
describe('Store', () => {
  ...
  it('resolves a gh-stack frontier and an explicit remote queue', async () => {
    const { directory, store } = await initializedStore();
    const stack = await makeGitStack(directory);
    const output = JSON.stringify({
      branches: [
        { name: 'stack/merged', head: stack.mergedSha, pr: { number: 10 } },
        { name: 'stack/closed', head: stack.closedSha, pr: { number: 13 } },
        { name: 'stack/open', head: stack.openSha, pr: { number: 11 } },
      ],
    });
    await withFakeGh({
      directory,
      ...
      operation: async (outputPath) => {
        ...
        await writeFile(outputPath, 'not a stack');
        expect(
          (
            await store.frontier.set({ repo: stack.repo, prs: [10, 11] })
          ).prs.map((pr) => pr.pr),
        ).toEqual([10, 11]);
        await expect(
        ...
        await expect(store.frontier.set({ repo: stack.repo })).rejects.toThrow(
          'Cannot resolve GitHub frontier',
        );
  ...
  it('rejects an unsubmitted branch and a local/remote head mismatch', async () => {
    const { directory, store } = await initializedStore();
    const stack = await makeGitStack(directory);
    await withFakeGh({
      directory,
      output: JSON.stringify({
        branches: [{ name: 'draft', head: stack.openSha }],
      }),
      operation: async (outputPath) => {
        await expect(store.frontier.set({ repo: stack.repo })).rejects.toThrow(
          'no valid submitted PR',
        );
        await writeFile(
          outputPath,
          JSON.stringify({
            branches: [
              { name: 'stack/open', head: stack.mergedSha, pr: { number: 11 } },
            ],
          }),
        );
        await expect(store.frontier.set({ repo: stack.repo })).rejects.toThrow(
