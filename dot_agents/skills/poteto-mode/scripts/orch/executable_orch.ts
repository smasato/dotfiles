...
function createProgram(io: Io): Command {
  ...
  const frontier = program
    .command('frontier')
    .description('manage a GitHub stack or explicit PR queue')
    .action(() => requireSubcommand(program));
  leaf(frontier, 'set', 'discover gh-stack or resolve an explicit queue')
    .addOption(
    ...
    .option(
      '--prs <n,...>',
      'explicit bottom-to-top pull request queue; otherwise read gh-stack',
      prList,
    )
