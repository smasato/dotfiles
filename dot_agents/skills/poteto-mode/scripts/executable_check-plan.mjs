...
const LANES = 'Ten lanes on `fast` at the PR head';
const SUB_BLOCKS = [
  'Depends on.',
  'Files.',
  'Build.',
  ...
  if (live) {
    ...
    for (const lane of lanes) {
      if (!lane.m) fail(lane.n, `${pr.title}: live box is not a lane`);
        ...
        fail(lane.n, `${pr.title}: lane ${lane.m[1]} names no screenshot`);
      else if (!lane.text.includes('Pass when'))
        fail(lane.n, `${pr.title}: lane ${lane.m[1]} has no pass predicate`);
    }
  }
...
if (closeIndex !== -1) {
  ...
  for (const s of tail) {
    if (!s.title.startsWith('Appendix'))
      fail(s.n, `"## ${s.title}" after Close the program is not an appendix`);
  }
