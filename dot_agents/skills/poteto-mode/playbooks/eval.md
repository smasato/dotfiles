...
6. **Verify the chain from evidence.** Locate candidate transcripts through pstack-runtime's scoped history reference. Inspect actual file-read events, including shell reads, and candidate artifacts. A claim to have read a skill is not evidence. If transcripts are inaccessible, grade artifact behavior separately and mark chain-following INCONCLUSIVE.
