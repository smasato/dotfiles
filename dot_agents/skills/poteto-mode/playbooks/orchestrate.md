...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through the host's delegation tool. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (within the host's actual nesting limit; the root handles deeper delegation when unavailable). Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Use the native runtime's workers. Each writer gets a separate worktree or disjoint files; include enough context and absolute paths. Verify the cwd before writes. Prefer fewer, broader workers. Use a distinct available model for independent verification when possible, and disclose when all reviewers share a model.
...
Create `.pstack/orchestrate/<project-slug>/` under the current workspace, or use an explicitly supplied writable store path. Set `ORCH_STORE` to its absolute path, or pass `--store <absolute-path>` to every command, before `orch init`. Keep runtime state out of version control. Every file has exactly one writer; owners publish facts, readers aggregate at read time. Run `bun ~/.agents/skills/poteto-mode/scripts/orch/orch.ts` for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
...
FORBIDDEN    no stack topology changes, no rebase, no force-push, no fixes outside scope, plus unit-specific bans
REPORT       status, branch, head SHA, PRs, verdict, what you actually ran, deviations,
             suggested follow-ups
STANDING     <preferences.md pasted verbatim>
```

Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape; a 4KB scaffold around a two-line edit costs more to write and obey than the edit. Local spawns may reference the standing-orders file by store path; include the actual text when a worker cannot read the path, and when resuming without retained context.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget with the host's concurrency and isolation limits, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and frontier delta).
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm it via pstack-runtime's monitoring reference, with a long heartbeat fallback), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Recompute `frontier.json` through `orch frontier set --repo <repo-dir>` after stack mutations. It reads `gh stack view --json` and resolves remote PR facts. Explicit queues use `--prs` with the frozen order. Record PRs, branches, head SHAs, generation, and the lowest unmerged PR.
- Exactly one owner per stack changes topology using the gh-stack skill; serialize those operations. Run expensive operations within the actual host's resource limits.
- Workers never rebase and leave stack mutations to the owner. Babysitters follow `playbooks/babysit.md`, one per stack, scoped to one immutable frontier generation; they report conflicts to the stacker rather than restacking.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the native host's task or agent status. Transcript mtime is not liveness.
...
- After a client restart, inspect actual task/process state; do not assume workers survived. Re-read standing orders and the ledger, verify branches and PRs, then reconnect to supported live tasks or create replacements with consolidated briefs.
