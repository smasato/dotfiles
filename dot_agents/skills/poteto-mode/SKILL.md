...
- Before commit → the `deslop` skill from the shared skills store (`/deslop`).
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → the matching control skill. The shared skills store includes `control-cli` (CLIs and TUIs) and `control-ui` (browser / Electron / web UIs). For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
...
**Delegates.** Use the host's pstack runtime for each bounded subtask. Give poteto workers this skill's path and the task scope. Routed workflows select their own reviewer prompts; preserve those. The parent owns the workflow.

**Delegation.** Resolve model roles and tool arguments through pstack-runtime. Include file pointers, write scope, completion criteria, and upstream findings. Continue useful parent work while children run. Do not assume cloud isolation, unlimited nesting, or access to sibling context.
...
- **Shipping.** The half after Babysit. Independently verifying a green stack, then landing the contiguous verified run with the verified gh-stack merge. `playbooks/shipping.md`.
...
- **Autopilot-stack.** A queue of changes built and verified with full autonomy, delivered as one linear reviewed gh-stack stack the operator lands herself ("autopilot-stack", "stack them, don't ship", "build the stack, I'll land it"). `playbooks/autopilot-stack.md`.
