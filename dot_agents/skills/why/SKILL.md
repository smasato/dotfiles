...
Before spawning investigators, discover connected tools through the host's tool catalog and any exposed search tool. Map them to evidence categories; report missing categories as unavailable.
...
- Delegate through the native pstack runtime.
- Resolve the `fast` role through the native runtime before spawning.
- Give investigators read-only briefs and access to the connected evidence tools. The native runtime defines tool availability; there is no portable read-only mode flag.
...
- Delegate through the native pstack runtime.
- Resolve the `judgment` role through the native runtime before spawning.
- Give the required tool access and an explicit write scope. (agent mode). The synthesizer's quality check spot-verifies citations, which can require MCP access. Readonly/Ask mode strips MCPs and defeats that.
