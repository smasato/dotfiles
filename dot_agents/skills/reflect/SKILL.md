...
- Substantive existing-skill edit: follow the [skill-authoring guide](../pstack-runtime/references/skill-authoring.md). Use installed `writing-for-agents`, or the guide's fallback if unavailable.
- `tune description: <skill path>`: revise the trigger against the missed case and representative non-matches. Use `skill-creator`'s optimization loop only if installed and useful.
- `new skill: <kebab-name>`: follow the same authoring guide, preserving the approved scope and invocation policy.

Run the available validators on touched skills as described in the guide. Missing `skill-creator` is not a blocker and does not authorize installation. Report manual checks separately from executed agent evaluations.
