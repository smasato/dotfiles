...
Spawn a single worker that explores and explains in one pass:
...
Once all explorers return, spawn a single worker to synthesize their findings into one coherent explanation:
...
After the explanation is complete, spawn one architectural critic per model in the native runtime's resolved review-panel, all in a single message.

For each critic:

- Delegate through the native pstack runtime.
- Use an actual model from the resolved review-panel, within the host's available models and permissions.
- Give a read-only brief.
